...
Dynamic results additionally expose the transition matrix in `my_result$B`
and a detailed decomposition in `my_result$dynamic`. For
`residual_family = "common_gamma"`, use
`my_result$dynamic$common_gamma` for the factor loadings and shape,
`my_result$dynamic$residual_covariance` for their implied covariance and
delta-method SEs, and `residual_M2`, `residual_M3`, and `residual_K4` for the
factor's cumulant contributions. The top-level `Psi_G` field is reserved for
the Gaussian residual family.
