...
``` 
my_model <- MCMedit(my_model,          # The base model
...
my_model <- MCMedit(my_model, pointer="A", name=c(7, 1), value=0) # Same for [7, 1]
```
...
coords <- list( c(3, 4, 5),
                c(2, 2, 2) )
my_model <- MCMedit(my_model, pointer="A", name=coords, value=0)
```

### By name
Lastly, coordinates can be dropped using their name. We will use this method to drop path `x1 -> x5` and `x5 -> x1`, which are labelled `b1_5` and `b5_1` respectively.
```
my_model <- MCMedit(my_model, pointer="A", name=c("b1_5", "b5_1"), value=0)
```

To verify our edits worked we can run `print(my_model, matrix="A")` again, which will now return the following:
``` 
     [,1] [,2] [,3]   [,4]   [,5]   [,6]   [,7]  
[1,] "0"  "0"  "0"    "0"    "0"    "0"    "0"   
[2,] "0"  "0"  "0"    "0"    "0"    "0"    "0"   
[3,] "a1" "0"  "0"    "b2_1" "b3_1" "b4_1" "0"  
[4,] "a1" "0"  "b1_2" "0"    "b3_2" "b4_2" "b5_2"
[5,] "a1" "0"  "b1_3" "b2_3" "0"    "b4_3" "b5_3"
[6,] "0"  "a2" "b1_4" "b2_4" "b3_4" "0"    "b5_4"
[7,] "0"  "a2" "0"    "b2_5" "b3_5" "b4_5" "0"   
```
...
my_model <- MCMedit(my_model, pointer="A", name=coords, value=new_names)
```
...
my_model <- MCMedit(my_model, pointer="A", name="a2", value=new_names)
```

Again we can verify these changes worked by running `print(my_model, matrix="A")`
``` 
     [,1]   [,2]   [,3]   [,4]   [,5]   [,6]   [,7]  
[1,] "0"    "0"    "0"    "0"    "0"    "0"    "0"   
[2,] "0"    "0"    "0"    "0"    "0"    "0"    "0"   
...
Currently, contraining parameters can only be done via coordinates. For example, to undo our previous change and contrain `a2_4` and `a2_5` to the same parameter, we can do the following:
``` 
my_model_single_a2 <- MCMedit(my_model, pointer="A", name=list(c(6, 7), c(2, 2)), value="a2")
```
...
Starting values can only be done by name (as the starting values are bound to the parameters themselves, not the matrices).
...
### Upper or lower bound of multiple parameters
