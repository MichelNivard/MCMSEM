...
The number of bootstrap iterations can be adjusted via `bootstrap_iter` (defaults to 200), and the number of chunks in a two-step bootstrap can be adjusted via `bootstrap_chunks` (defaults to 1000).

```
my_result <- MCMfit(base_model, data_summary, se_type='two-step',
                    bootstrap_iter=100, bootstrap_chunks=500)
```
...
my_result <- MCMfit(base_model, data_summary, optimizers=c('sgd', 'lbfgs'), learning_rate=c(0.1, 0.9), optim_iters=c(100, 25))   # Optimizer sequence
```
...
MCMSEM uses a smooth penalty for parameter bounds because the available optimizers do not all support hard bounds. Bounds are enabled by default. Set `use_bounds = FALSE` only when you intentionally want an unconstrained fit, or adjust `outofbounds_penalty` to change the penalty strength.
