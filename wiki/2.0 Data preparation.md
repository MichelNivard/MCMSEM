...
From MCMSEM 0.27.0 onward, `S.m` is the estimated sampling covariance of the
raw central-moment vector. Its third- and fourth-order casewise values use
influence-function corrections for estimating the sample means. This metadata
is stored in new `.mcmdata` files. Dynamic WLS or SE calculations reject older
summaries without the correction; recreate those summaries from raw data.
...
mod <- MCMmodel(mcmsem_summary, kernel = "contemporaneous")
```
