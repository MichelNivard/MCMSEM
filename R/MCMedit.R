# This is all code to get mcedit to work the way I want to
# This may seem a bit overkill, but I think it looks nicer than having to edit everything manually, e.g.
# mcmmodelinstance$named_matrices[["A"]][mcmmodelinstance$named_matrices[["A"]] == "a1"] <- c(1, 2)
setGeneric("mcmedit<-", function(x, y, z, value) standardGeneric("mcmedit<-"))
setMethod("mcmedit<-", sig = "ANY", function(x, y, z, value) {
  UseMethod("mcmedit<-")
})

"mcmedit<-.default" <- function(x, y, z, value) {
  stop(paste("mcmedit assignment (mcmedit<-) method not defined for class", class(x)))
}

"mcmedit<-.mcmmodelclass" <- function(x, y, z, value) {
